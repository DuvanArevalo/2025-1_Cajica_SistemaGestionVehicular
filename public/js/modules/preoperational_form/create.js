document.addEventListener('DOMContentLoaded', function () {
    ...
    // Solo deshabilitar el select de vehículos si no hay tipo seleccionado y no está ya deshabilitado por ser conductor
    if (!vehicleSelect.disabled && !vehicleTypeSelect.value) {
        vehicleSelect.disabled = true;
    }

    // Filtrar vehículos por tipo
    function filterVehiclesByType(typeId) {
        // Si el select no está deshabilitado por ser conductor (tiene el atributo disabled),
        // entonces podemos manipular su estado
        if (!vehicleSelect.hasAttribute('disabled')) {
            // Habilitar el select cuando hay un tipo seleccionado
            vehicleSelect.disabled = !typeId;
        }

        // Guardar el valor actual antes de filtrar
    ...
    function initializeForm() {
        // Buscar si hay un vehículo preseleccionado (ya sea por hidden input o por URL)
        const hiddenVehicleInput = document.querySelector('input[type="hidden"][name="vehicle_id"]');
        const urlParams = new URLSearchParams(window.location.search);
        const preselectedVehicleId = hiddenVehicleInput ? hiddenVehicleInput.value : urlParams.get('vehicle_id');

        if (preselectedVehicleId) {
            // Seleccionar la opción correspondiente
            const option = Array.from(vehicleSelect.options).find(opt => opt.value === preselectedVehicleId);
            if (option) {
